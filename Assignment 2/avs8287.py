...
############################################ Assignment 2 ##########################################
def processRPN(stack, token):
  if(token.isdigit() == True):         # When you find a # add it to stack
    stack.push(token)                 
  if(token == '+'):                    # Pop last 2 numbers from stack,  
    numb1 = int(stack.pop())           # Perform op. on them,      
    numb2 = int(stack.pop())           # Push result
    stack.push(numb2 + numb1)
  if(token == '-'):
    numb1 = int(stack.pop())                     # Push result
    numb2 = int(stack.pop())
    stack.push(numb2 - numb1)
  if(token == '*'):
    numb1 = int(stack.pop())                     # Push result
